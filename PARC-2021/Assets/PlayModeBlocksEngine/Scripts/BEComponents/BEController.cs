...
public class BEController : MonoBehaviour
{
    ...
    {
        Robot.soundfreq = 39;
        Robot.beAudioSource.clip = Robot.ready;
        Robot.beAudioSource.Play();
        yield return new WaitForSeconds(1f);
        Robot.soundfreq = 79;
        Robot.beAudioSource.clip = Robot.set;
        Robot.beAudioSource.Play();
        yield return new WaitForSeconds(1f);
        Robot.soundfreq = 100;
        Robot.beAudioSource.clip = Robot.go;
        Robot.beAudioSource.Play();
        Robot.isGo = true;
        yield return new WaitForSeconds(1f);
        trafficLight.transform.gameObject.SetActive(false);
        Robot.soundfreq = 0;
        Robot.isGo = false;
    }
       ...
       /* if(SceneManager.GetActiveScene().name == "Main")
        {
            ...
            trafficLight.SetBool("Traffic Start", true);
            StartCoroutine(FindObjectOfType<GameHandler>().Timer());
        }
        
        if(SceneManager.GetActiveScene().name == "Practice")
        {
            if (FindObjectOfType<PresetHandler>().startcount)
            {
                StartCoroutine(PracticeStart());
                FindObjectOfType<PresetHandler>().CountDown.SetActive(true);
            }
        }*/
